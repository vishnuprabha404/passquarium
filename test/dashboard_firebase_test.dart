  ...
  group('🏠 Dashboard & Firebase Security Tests', () {
    ...
    group('🔍 Password Search & Dashboard Tests', () {
      test('should search passwords by website name', () async {
        ...
        final testPasswords = [
          await _createPasswordEntry('gmail.com', testEmail, gmailPassword, testPassword),
          await _createPasswordEntry('github.com', 'testdev', githubPassword, testPassword),
          await _createPasswordEntry('bankofamerica.com', 'customer123', bankPassword, testPassword),
          await _createPasswordEntry('google.com', testEmail, 'GooglePass123!', testPassword),
        ];
        ...
        expect(gmailResults.length, equals(1), reason: 'Should find Gmail entry');
        expect(githubResults.length, equals(1), reason: 'Should find GitHub entry');
        expect(bankResults.length, equals(1), reason: 'Should find bank entry');
        expect(googleResults.length, equals(2), reason: 'Should find both Google entries');
        expect(noResults.length, equals(0), reason: 'Should find no nonexistent entries');
      ...
      test('should search passwords by username', () {
        ...
        expect(userResults.length, equals(1), reason: 'Should find entry by username');
        expect(otherResults.length, equals(1), reason: 'Should find other user entry');
      ...
      test('should extract domain from URLs correctly', () {
        ...
        final testCases = [
          ...
          ('https://secure.bankofamerica.com/login', 'secure.bankofamerica.com'),
    ...
    group('🔒 Firebase Security Tests - NO SENSITIVE DATA', () {
      test('should ensure NO plaintext passwords stored in Firebase data', () async {
      ...
      test('should ensure NO master password hashes stored in Firebase', () async {
        ...
        final testEntries = [
          await _createPasswordEntry('example1.com', 'user1', 'Pass123!', masterPassword),
          await _createPasswordEntry('example2.com', 'user2', 'Secret456@', masterPassword),
          await _createPasswordEntry('example3.com', 'user3', 'Secure789#', masterPassword),
        ];
        
        print('Master password hash: ${masterHash.substring(0, 32)}...');
        print('Testing ${testEntries.length} password entries for hash leakage...');
      ...
      test('should simulate complete dashboard workflow', () async {
        ...
        for (final (searchTerm, expectedCount) in searchTests) {
          ...
          expect(results.length, equals(expectedCount), 
                reason: 'Search for "$searchTerm" should return $expectedCount results');
        }
        
        // Step 3: Test password retrieval and decryption
        print('\n3. Testing password retrieval...');
        
        for (final entry in passwordEntries.take(3)) { // Test first 3
        ...
        for (final (_, _, _, password) in websiteData) {
          expect(firebaseString.contains(password), false, 
                reason: 'Firebase data should not contain plaintext password: $password');
        }
      ...
      test('should categorize passwords correctly', () async {
        ...
        for (final (category, website, password) in categorizedPasswords) {
          final entry = PasswordEntry(
            ...
            domain: website,
            username: testEmail,
            encryptedPassword: await encryptionService.encryptPassword(password, masterPassword),
            createdAt: DateTime.now(),
            updatedAt: DateTime.now(),
            category: category,
          );
        ...
        expect(gmailResults.length, equals(1), reason: 'Should find Gmail entry');
        expect(chaseResults.length, equals(1), reason: 'Should find Chase entry');
    ...
    group('🛡️ Security Audit Tests', () {
      test('should audit password strength in dashboard', () {
        ...
        for (final (description, password) in testPasswords) {
          ...
          final strengthDesc = passwordService.getPasswordStrengthDescription(strength);
      ...
      test('should verify Firebase data structure security', () async {
        ...
        expect(firebaseMap.containsKey('website'), true, reason: 'Should have website');
        expect(firebaseMap.containsKey('username'), true, reason: 'Should have username');
        expect(firebaseMap.containsKey('encryptedPassword'), true, reason: 'Should have encrypted password');
        expect(firebaseMap.containsKey('createdAt'), true, reason: 'Should have creation date');
        
        // Verify no sensitive keys
        expect(firebaseMap.containsKey('password'), false, reason: 'Should NOT have plaintext password key');
        expect(firebaseMap.containsKey('masterPassword'), false, reason: 'Should NOT have master password key');
        expect(firebaseMap.containsKey('masterHash'), false, reason: 'Should NOT have master hash key');
        expect(firebaseMap.containsKey('pin'), false, reason: 'Should NOT have PIN key');
    ...
    group('🔍 Firebase Search Security Tests', () {
      test('should ensure search functionality works without exposing sensitive data', () async {
        ...
        print('Added ${sensitivePasswords.length} entries with sensitive passwords');
          ...
          for (final result in results) {
            ...
            for (final (_, _, sensitivePass) in sensitivePasswords) {
              expect(dataString.contains(sensitivePass), false,
                    reason: 'Search results should not contain plaintext password: $sensitivePass');
            }
      ...
      test('should verify password retrieval through search is secure', () async {
        ...
        // Add test entries
        final entries = [
          await _createPasswordEntry('test1.com', 'user1', testPassword1, masterPassword),
          await _createPasswordEntry('test2.com', 'user2', testPassword2, masterPassword),
        ];
        ...
        expect(searchResults.length, equals(1), reason: 'Should find one result');
        ...
        try {
          ...
          print('Correctly failed with wrong master password: ${e.toString().substring(0, 50)}...');
        }
