...
void main() {
  group('🔒 Super Locker Encryption & Password Tests', () {
    ...
    group('🔐 Wrong Credentials Tests (First)', () {
      test('should identify wrong email format', () {
        ...
        for (final email in invalidEmails) {
          ...
          expect(email != testEmail, true,
              reason: 'Should be different from correct email');
        }
      ...
      test('should identify wrong passwords', () {
        ...
        for (final password in invalidPasswords) {
          ...
          expect(password != testPassword, true,
              reason: 'Should be different from correct password');
        }
      ...
      test('should identify wrong Windows PIN', () {
        const invalidPins = [wrongPin, '0000', '1234', '', '123456789', 'abcd'];

        for (final pin in invalidPins) {
          print('Testing invalid PIN: $pin');
          expect(pin != testWindowsPin, true,
              reason: 'Should be different from correct PIN');
        }
    ...
    group('✅ Correct Credentials Tests (Second)', () {
      test('should recognize correct email', () {
        ...
        expect(testEmail.contains('@'), true,
            reason: 'Valid email should contain @');
        expect(testEmail.contains('.'), true,
            reason: 'Valid email should contain domain');
      ...
      test('should recognize correct Windows PIN', () {
        ...
        expect(testWindowsPin.length, equals(8),
            reason: 'PIN should be 8 digits');
        expect(RegExp(r'^\d+$').hasMatch(testWindowsPin), true,
            reason: 'PIN should be numeric');
    ...
    group('🔒 Password Encryption & Decryption Tests', () {
      test(
          'should encrypt and decrypt password correctly with test credentials',
          () async {
        ...
        try {
          ...
          expect(encryptedPassword, isNot(equals(testSitePassword)),
              reason: 'Encrypted should differ from original');
          ...
          expect(decryptedPassword, equals(testSitePassword),
              reason: 'Decrypted should match original');
      ...
      test('should fail decryption with wrong master password', () async {
        ...
        try {
          ...
          print(
              '✅ Wrong master password correctly rejected: ${e.toString().split('\n').first}');
          expect(e.toString(), contains('failed'),
              reason: 'Should throw decryption error');
        }
      });

      test('should generate unique encrypted outputs for same password',
          () async {
        ...
        final decrypted1 =
            await encryptionService.decryptPassword(encrypted1, masterPassword);
        final decrypted2 =
            await encryptionService.decryptPassword(encrypted2, masterPassword);
    ...
    group('🔑 Master Password Hash Tests', () {
      test('should hash master password consistently', () {
        ...
        expect(hash1.length, greaterThan(30),
            reason: 'Hash should be substantial length');
      ...
      test('should verify master password hash correctly', () async {
        ...
        final isValidCorrect =
            await encryptionService.verifyMasterPassword(masterPassword, hash);
        final isValidWrong = await encryptionService.verifyMasterPassword(
            wrongMasterPassword, hash);
    ...
    group('🎯 Password Strength Tests', () {
      test('should calculate password strength correctly', () {
        ...
        for (final (password, description) in passwords) {
          ...
          expect(strength, inInclusiveRange(0, 100),
              reason: 'Strength should be 0-100%');
        }

        // Test password should have reasonable strength
        final testPasswordStrength =
            encryptionService.calculatePasswordStrength(testPassword);
        expect(testPasswordStrength, greaterThan(25),
            reason: 'Test password should have decent strength');
      ...
      test('should generate secure passwords', () {
        ...
        expect(strength1, greaterThan(60),
            reason: 'Generated password should be strong');
    ...
    group('🔄 Integration Simulation Tests', () {
      test('should simulate complete password storage flow', () async {
        ...
        final encryptedPassword = await encryptionService.encryptPassword(
            websitePassword, masterPassword);
