  ...
  group('Super Locker Tests', () {
    ...
    group('🔐 Authentication Service Tests', () {
      test('should reject wrong email during sign in', () async {
        try {
          final result = await authService.signInWithEmail(wrongEmail, testPassword);
      ...
      test('should reject wrong password during sign in', () async {
        try {
          final result = await authService.signInWithEmail(testEmail, wrongPassword);
      ...
      test('should reject invalid email format', () async {
        ...
        try {
          final result = await authService.signInWithEmail(invalidEmail, testPassword);
      ...
      test('should reject weak password during sign up', () async {
        ...
        try {
          final result = await authService.signUpWithEmail(testEmail, weakPassword);
      ...
      test('should accept valid credentials for sign in', () async {
        ...
        print('Testing valid credentials: $testEmail with password $testPassword');
        ...
        try {
          ...
          if (result) {
            ...
            expect(authService.userEmail, testEmail, reason: 'Should store correct email');
          } else {
    ...
    group('🔑 Master Password Tests', () {
      test('should reject wrong master password', () async {
        ...
        try {
          final result = await authService.verifyMasterPassword(wrongMasterPassword);
      ...
      test('should check master password existence', () async {
        try {
          ...
          expect(hasPassword, isA<bool>(), reason: 'Should return boolean value');
        } catch (e) {
          expect(e.toString(), contains('authenticated user'), 
    ...
    group('🔒 Encryption Service Tests', () {
      ...
      test('should calculate password strength correctly', () {
        ...
        final weakStrength = encryptionService.calculatePasswordStrength(weakPassword);
        final strongStrength = encryptionService.calculatePasswordStrength(strongPassword);
      ...
      test('should verify master password hash correctly', () async {
        ...
        final isInvalid = await encryptionService.verifyMasterPassword(
          wrongPassword, 
          hash
        );
    ...
    group('📝 Password Service Tests', () {
      ...
      test('should search passwords correctly', () {
        ...
        expect(results.length, equals(1), reason: 'Should find one Google entry');
        expect(results.first.website, equals('google.com'));
        
        final emptyResults = passwordService.searchPasswords('nonexistent');
        expect(emptyResults.length, equals(0), reason: 'Should find no results for nonexistent');
      });
    ...
    group('🔄 Integration Tests', () {
      test('should handle complete authentication flow', () async {
        ...
        try {
          ...
          print('⚠️ Correct credentials test failed (expected in unit test): ${e.toString().split('\n').first}');
        }
      ...
      test('should encrypt and store password entry', () async {
        ...
        try {
          ...
          print('Encrypted password: ${entry.encryptedPassword.substring(0, 20)}...');
    ...
    group('🛡️ Security Tests', () {
      test('should not store passwords in plaintext', () {
        ...
        encryptionService.encryptPassword(plainPassword, masterPassword).then((encrypted) {
    ...
    group('📊 Performance Tests', () {
      test('should encrypt/decrypt passwords efficiently', () async {
        ...
        for (int i = 0; i < 10; i++) {
          final encrypted = await encryptionService.encryptPassword(password, masterPassword);
          final decrypted = await encryptionService.decryptPassword(encrypted, masterPassword);
          expect(decrypted, equals(password));
        }
        
        stopwatch.stop();
        print('10 encrypt/decrypt operations took: ${stopwatch.elapsedMilliseconds}ms');
